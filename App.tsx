      ...
      <div className="absolute bottom-6 left-0 right-0 px-6 z-50 flex justify-center pointer-events-none">
        <div className="pointer-events-auto bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 p-3 rounded-[32px] shadow-2xl flex gap-3 max-w-[90vw] overflow-x-auto scroll-custom no-scrollbar">
          {vectors.map((v) => {
            ...
            return (
              <div key={v.id} role="group" aria-label={`Tarjeta de vector ${v.label}`} className={`relative min-w-[110px] p-3 rounded-[24px] border transition-all duration-300 flex flex-col items-center gap-1.5 group ${isSelected ? 'bg-slate-800 border-blue-500 -translate-y-2 shadow-xl' : 'bg-slate-900/40 border-slate-800 hover:border-slate-600'}`} style={isSelected ? { borderColor: v.color, boxShadow: `0 10px 30px ${v.color}22` } : {}}>
                <button aria-label={`Seleccionar vector ${v.label}`} onClick={() => handleSelection(v.id)} className="absolute inset-0 z-0 rounded-[24px] focus-visible:ring-2 focus-visible:ring-blue-500 focus:outline-none"></button>
                <button aria-label={`Eliminar vector ${v.label}`} title="Eliminar vector" onClick={(e) => { e.stopPropagation(); deleteVector(v.id); }} className="absolute -top-1 -right-1 w-5 h-5 bg-red-500/90 text-white rounded-full opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-red-400 focus:outline-none transition-opacity flex items-center justify-center text-[8px] z-20">✕</button>
                {isSelected && <div className="absolute -top-1.5 -left-1.5 w-6 h-6 rounded-full flex items-center justify-center font-black text-[9px] text-white shadow-lg z-10 pointer-events-none animate-in zoom-in" style={{ backgroundColor: v.color }}>{selectionIndex + 1}</div>}
                <div className="w-8 h-8 rounded-xl flex items-center justify-center font-black text-sm z-10 pointer-events-none" style={{ backgroundColor: `${v.color}22`, color: v.color }}>{v.label}</div>
