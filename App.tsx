      ...
      <div className="absolute bottom-6 left-0 right-0 px-6 z-50 flex justify-center pointer-events-none">
        <div className="pointer-events-auto bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 p-3 rounded-[32px] shadow-2xl flex gap-3 max-w-[90vw] overflow-x-auto scroll-custom no-scrollbar">
          {vectors.map((v) => {
            ...
            return (
              <div role="group" key={v.id} className={`relative min-w-[110px] p-3 rounded-[24px] border transition-all duration-300 flex flex-col items-center gap-1.5 group ${isSelected ? 'bg-slate-800 border-blue-500 -translate-y-2 shadow-xl' : 'bg-slate-900/40 border-slate-800 hover:border-slate-600'}`} style={isSelected ? { borderColor: v.color, boxShadow: `0 10px 30px ${v.color}22` } : {}}>
                <button
                  aria-label={`${isSelected ? 'Deseleccionar' : 'Seleccionar'} vector ${v.label}`}
                  onClick={() => handleSelection(v.id)}
                  className="absolute inset-0 w-full h-full cursor-pointer z-0 focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:outline-none rounded-[24px]"
                />
                ...
                <div className="pointer-events-none contents">
                  ...
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center font-black text-sm z-10" style={{ backgroundColor: `${v.color}22`, color: v.color }}>{v.label}</div>
