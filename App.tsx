      ...
      <div className="absolute bottom-6 left-0 right-0 px-6 z-50 flex justify-center pointer-events-none">
        <div className="pointer-events-auto bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 p-3 rounded-[32px] shadow-2xl flex gap-3 max-w-[90vw] overflow-x-auto scroll-custom no-scrollbar">
          {vectors.map((v) => {
            ...
            return (
              <div key={v.id} role="button" tabIndex={0} aria-pressed={isSelected} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleSelection(v.id); } }} onClick={() => handleSelection(v.id)} className={`relative min-w-[110px] p-3 rounded-[24px] border cursor-pointer transition-all duration-300 flex flex-col items-center gap-1.5 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 ${isSelected ? 'bg-slate-800 border-blue-500 -translate-y-2 shadow-xl' : 'bg-slate-900/40 border-slate-800 hover:border-slate-600'}`} style={isSelected ? { borderColor: v.color, boxShadow: `0 10px 30px ${v.color}22` } : {}}>
                <div role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); deleteVector(v.id); } }} aria-label="Eliminar vector" title="Eliminar vector" onClick={(e) => { e.stopPropagation(); deleteVector(v.id); }} className="absolute -top-1 -right-1 w-5 h-5 bg-red-500/90 text-white rounded-full opacity-0 group-hover:opacity-100 focus-visible:opacity-100 transition-opacity flex items-center justify-center text-[8px] z-10 outline-none focus-visible:ring-2 focus-visible:ring-white">✕</div>
