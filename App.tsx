          ...
          {vectors.map((v) => {
            ...
            return (
              <div key={v.id} role="group" aria-label={`Vector ${v.label}`} className={`relative min-w-[110px] p-3 rounded-[24px] border transition-all duration-300 flex flex-col items-center gap-1.5 group ${isSelected ? 'bg-slate-800 border-blue-500 -translate-y-2 shadow-xl' : 'bg-slate-900/40 border-slate-800 hover:border-slate-600'}`} style={isSelected ? { borderColor: v.color, boxShadow: `0 10px 30px ${v.color}22` } : {}}>
                <button aria-label={`Seleccionar vector ${v.label}`} onClick={() => handleSelection(v.id)} className="absolute inset-0 z-0 rounded-[24px] focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none" />
                <button aria-label={`Eliminar vector ${v.label}`} title={`Eliminar vector ${v.label}`} onClick={(e) => { e.stopPropagation(); deleteVector(v.id); }} className="absolute -top-1 -right-1 w-5 h-5 bg-red-500/90 text-white rounded-full opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:outline-none transition-opacity flex items-center justify-center text-[8px] z-20">✕</button>
