            ...
            {aiLoading ? (
              ...
              <>
                <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                  ...
                  <button aria-label="Cerrar chat de IA" title="Cerrar chat de IA" onClick={() => { setExplanation(null); setChatHistory([]); }} className="p-1.5 hover:bg-slate-800 rounded-lg text-slate-500 transition-colors">✕</button>
                </div>
                ...
                <form onSubmit={handleAskQuestion} className="p-3 border-t border-slate-800 bg-slate-900/80 flex gap-2">
                  ...
                  <button aria-label="Enviar pregunta a la IA" title="Enviar pregunta a la IA" disabled={!userQuestion.trim() || isAsking} type="submit" className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white p-2 rounded-xl transition-all active:scale-90"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg></button>
                </form>
              </>
            )}
          </div>
